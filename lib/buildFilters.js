 ...
 */
function buildFilters({ fps, width, widthFactor = null, colors, dither, keepOriginalDimensions = false, widthOverride = false }) {
  // keepOriginalDimensions: SAR fix only, no resize.
  // widthFactor: scale to a fraction of source width (e.g. 0.5); ffmpeg resolves iw at runtime.
  ...
  const scale = keepOriginalDimensions
    ...
    : widthFactor != null
      ? `scale=iw*sar:ih,scale=iw*${widthFactor}:-2:flags=lanczos,setsar=1`
      : widthOverride
