...
function buildVideoArgs(input, settings, output) {
  const { keepOriginalDimensions, widthOverride, width, widthFactor, codec, crf, preset, audioBitrate, fps } = settings;

  const vfParts = [];
  if (fps) vfParts.push(`fps=${fps}`);
  if (widthFactor != null) {
    vfParts.push(`scale=iw*${widthFactor}:-2:flags=lanczos`);
